...
import { removeFavMovie } from '../../actions/actions';
...
class ProfileView extends React.Component {
    ...
    getUser(token) {
        ...
        axios.get(`https://myflixnetflix.herokuapp.com/users/${Username}`, {
    ...
    updateUser = (e) => {
        ...
        axios.put(`https://myflixnetflix.herokuapp.com/users/${Username}`, 
        {
    ...
    removeFromFavorite = (event, movie) => {
        ...
        axios
          .delete(
            `https://myflixnetflix.herokuapp.com/users/${username}/movies/${movie._id}`,
            {
              headers: { Authorization:`Bearer ${token}`}
            }
          )
          .then((res) => {
            ...
            this.props.removeFavMovie(res?.data)
          })
          .catch((err) => {
            console.log(err);
      })
    ...
    removeUser() {
        const Username = localStorage.getItem('user');
        const token = localStorage.getItem('token');
        axios.delete(`https://myflixnetflix.herokuapp.com/users/${Username}`, {
            headers: { Authorization: `Bearer ${token}` }
        })
        .then(() => {
...
export default connect(mapStateToProps, { removeFavMovie })(ProfileView);